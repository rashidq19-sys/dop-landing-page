...
const MODULES = [
  ...
  { key: "portal", name: "Driver Portal", short: "Driver app", blurb: "Every driver gets their own app — view their rota, download payslips, check their scorecard, and submit van inspections. No more WhatsApp forwarding. Everything they need, in one place, on their phone.", icon: Smartphone, saved: "3hr/wk" },
...
function ModulePreview({ moduleKey }: { moduleKey: ModuleKey }) {
  if (moduleKey === "portal") {
    return <img src="/images/driver-portal.png" className="max-h-[220px] sm:max-h-[440px] rounded-[22px] shadow-[0_25px_60px_-12px_rgba(17,17,19,0.28)]" alt="Driver portal" />;
  }
  if (moduleKey === "payroll") {
    return <img src="/images/payroll-phone.png" className="max-h-[220px] sm:max-h-[440px] rounded-[22px] shadow-[0_25px_60px_-12px_rgba(17,17,19,0.28)]" alt="Payroll" />;
  }
  if (moduleKey === "scorecards") {
    return <img src="/images/scorecard.png" className="max-h-[220px] sm:max-h-[440px] rounded-[22px] shadow-[0_25px_60px_-12px_rgba(17,17,19,0.28)]" alt="Scorecards" />;
  }
  if (moduleKey === "rota") {
    return <img src="/images/rota-phone-v2.png" className="max-h-[220px] sm:max-h-[440px] rounded-[22px] shadow-[0_25px_60px_-12px_rgba(17,17,19,0.28)]" alt="Rota" />;
  }
  if (moduleKey === "dispatch") {
    return <img src="/images/dispatch-deployment-plan.png" className="max-h-[220px] sm:max-h-[440px] rounded-[22px] shadow-[0_25px_60px_-12px_rgba(17,17,19,0.28)]" alt="Deployment Plan" />;
  }
  if (moduleKey === "damage") {
    return <img src="/images/van-checks.png" className="max-h-[220px] sm:max-h-[440px] rounded-[22px] shadow-[0_25px_60px_-12px_rgba(17,17,19,0.28)]" alt="Van Checks" />;
  }
  if (moduleKey === "compliance") {
    return <img src="/images/compliance.png" className="max-h-[220px] sm:max-h-[440px] rounded-[22px] shadow-[0_25px_60px_-12px_rgba(17,17,19,0.28)]" alt="Compliance" />;
  }
  if (moduleKey === "sdd") {
    return <img src="/images/sdd.png" className="max-h-[220px] sm:max-h-[440px] rounded-[22px] shadow-[0_25px_60px_-12px_rgba(17,17,19,0.28)]" alt="Same-Day Delivery" />;
  }
  return <img src="/images/tracking.png" className="max-h-[220px] sm:max-h-[440px] rounded-[22px] shadow-[0_25px_60px_-12px_rgba(17,17,19,0.28)]" alt="Tracking" />;
}
...
export default function FeaturesSection() {
  ...
  return (
    <section ref={sectionRef} id="features" className="bg-white py-[70px] sm:py-[100px] border-b border-border">
      <div className="max-w-[1280px] mx-auto px-4 sm:px-8">
        {/* Header */}
        <div className="mb-12">
          <div className="text-[11px] font-semibold text-brand uppercase tracking-[0.14em] mb-3.5">— PLATFORM</div>
          <div className="flex items-end justify-between gap-10 flex-wrap">
            <h2 className="text-[30px] sm:text-[52px] font-extrabold text-[#111113] tracking-[-0.035em] leading-[1.02]">Nine modules. One login. Zero spreadsheets.</h2>
        ...
        {/* Tab buttons */}
        <div className="bg-background rounded-[16px] border border-border p-2 grid grid-cols-3 sm:grid-cols-9 gap-1 mb-6">
          {MODULES.map((mod, i) => {
            const Ic = mod.icon;
            return (
              <button key={mod.key} onClick={() => handleTabClick(i)}
                className={`py-3 sm:py-4 px-1 sm:px-2 flex flex-col items-center gap-1.5 sm:gap-2 rounded-[11px] text-[12px] font-semibold transition-all duration-150 relative ${
                  active === i
                ...
                {active === i && (
                  <div className="hidden sm:block absolute bottom-[-9px] left-1/2 -translate-x-1/2 w-0 h-0 border-l-[7px] border-l-transparent border-r-[7px] border-r-transparent border-t-[7px] border-t-brand" />
                )}
              </button>
            );
          })}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.1fr] gap-4">
          <div className="bg-background rounded-[16px] border border-border p-5 sm:p-9 flex flex-col">
            <div className="text-[11px] text-[#6C6C72] uppercase tracking-[0.1em]">MODULE · 0{active + 1}/09 · {m.short.toUpperCase()}</div>
            <h3 className="text-[30px] sm:text-[44px] font-extrabold text-[#111113] tracking-[-0.03em] leading-[1.02] mt-3">{m.name}</h3>
          ...
          <div className="bg-gradient-to-b from-background to-border/40 rounded-[16px] border border-border p-5 sm:p-9 min-h-[260px] sm:min-h-[480px] flex items-center justify-center">
