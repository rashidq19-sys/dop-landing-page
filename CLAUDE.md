...
- `[IMPORTANT]` **Brand colours come from `client/public/logo.png`, not from taste.** The blue is `#1879f1` and the mark's square is `#121835`; both were sampled from that file on 2026-08-24, when the site was still running `oklch(0.55 0.2 255)` — visibly more indigo than the logo sitting next to it. `--color-brand` is the *fill* colour; small text on white uses `--color-brand-dark` (`#0b63d6`), because the logo blue alone measures 4.2:1 against white and misses the 4.5:1 floor. If a new colour needs to "look like DSPOps", sample the logo rather than eyeballing it.
