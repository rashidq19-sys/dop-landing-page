...
- `[CRITICAL]` **The mobile chat sheet listens to BOTH `visualViewport.resize` AND `window.resize` — neither is redundant.** iOS does not shrink the layout viewport when the software keyboard opens; it shrinks the *visual* viewport and scrolls the page underneath, so a fixed panel ends up behind the keyboard while Safari drags the page about trying to reveal the focused input. `useVisualViewportSheet` in `ChatbotWidget.tsx` pins the sheet to `window.visualViewport` instead. But **Android Chrome resizes the layout viewport and only fires `window.resize`** — drop that listener and the sheet stays stuck at its pre-keyboard height on Android. Companion hooks in the same file: `useIsMobileViewport` (uses `matchMedia` on width, so the keyboard can't retrigger it) and `useBodyScrollLock` (`position: fixed` on body — the only lock iOS respects — restoring the exact scroll position on close). Below 640px the panel is a full-height sheet; at 640px and above it is the original floating panel, unchanged.
