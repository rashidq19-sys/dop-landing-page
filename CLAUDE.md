...
- `[CRITICAL]` **`client/public/llms.txt` is load-bearing twice over — do not delete it, move it, or let it go stale.** It is (a) the plain-text product map for AI assistants, which mostly do not run JS, and (b) **the factual ground truth the weekly blog agent is required to read before writing any sentence about what DSPOps does.** The second job is the one that fails silently: the agent's routine tells it every product claim must be traceable to a line in `llms.txt` or a feature page, so if the file disappears the guardrail disappears with it and the agent goes back to inventing capabilities — which is exactly what happened on 2026-09-07 (a post claiming per-mode pay rules calculating into one pay run, and "cycle" used to mean bikes, when app-wide `cycle` means same-day delivery). **When a real capability ships, update `llms.txt` in the same change** — otherwise the blog agent, the site chatbot's `SYSTEM_PROMPT` and every AI assistant describing DSPOps are all working from a stale picture. The routine is "Weekly DSPOps Blog Post" (`trig_01Xx9fMJDvkm2scjMawJBjfH`, Mondays 08:00 UTC); its prompt is editable at https://claude.ai/code/routines and it also requires the agent to list, in its final report, every product claim it made with the supporting line.
